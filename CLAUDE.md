...
**Status (2026-04-30):** Active. The legacy parser runs cleanly against the current Retrosheet corpus (~200K games in 14s) modulo 3 known game-level errors deferred for later (BSN191409102, ATN193807032, BIR194806210). A `pest`-grammar rewrite remains an in-flight option but is not blocking. Confirm the current direction with the user before doing significant parser work.
...
Full-corpus validation remains the final gate: run the binary against the full Retrosheet corpus and diff Parquet output downstream.
